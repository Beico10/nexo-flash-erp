...
func Wire(cfg Config) (*Container, error) {
	...
	enterpriseRepo := memory.NewEnterpriseRepo()
	...
	enterpriseSvc := enterprise.NewService(enterpriseRepo)
	...
	return &Container{
		...
		EnterpriseHandler:  handlers.NewEnterpriseHandler(enterpriseSvc),
