-- =============================================================================
-- NEXO ONE ERP — Migração 003: Painel de Aprovação IA + Pagamentos BaaS
-- =============================================================================
...
CREATE INDEX idx_pix_tenant ON baas_pix_charges(tenant_id);
CREATE INDEX idx_pix_status ON baas_pix_charges(tenant_id, status) WHERE status = 'pending';
CREATE INDEX idx_boleto_tenant ON baas_boletos(tenant_id);
...
CREATE OR REPLACE VIEW ai_pending_panel AS
SELECT
    s.id,
    s.tenant_id,
    s.suggestion_type,
...
-- =============================================================================
-- MÓDULO: LOGÍSTICA — Tabela de Contratos
-- =============================================================================
...
CREATE POLICY logi_contract_iso ON logistics_contracts USING (tenant_id = current_tenant_id());

CREATE INDEX idx_logi_contract_tenant ON logistics_contracts(tenant_id);
...
COMMENT ON FUNCTION resolve_contract IS
    'Hierarquia de contratos: específico do embarcador > tabela geral.';
