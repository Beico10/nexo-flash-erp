...
import { isDemoMode, promptLogin, DEMO_MECHANIC_ORDERS } from '@/lib/demo'

interface ServiceOrder {
  id: string; tenant_id: string; number: string; vehicle_plate: string; vehicle_km: number
  vehicle_model: string; vehicle_year: number; customer_id: string; customer_phone: string
  status: string; complaint: string; diagnosis: string; created_at: string; updated_at: string
}
...
export default function MechanicPage() {
  ...
  const fetchOrders = useCallback(async () => {
    if (isDemoMode()) {
      setOrders(DEMO_MECHANIC_ORDERS as ServiceOrder[])
      setLoading(false)
      return
    }
    const token = getToken()
    ...
    <div className="space-y-5 animate-fade-in">
      {isDemoMode() && (
        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 12, padding: '10px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#92400E' }}>Demonstracao de Mecanica</span>
            <span style={{ fontSize: 12, color: '#64748b' }}>Explore a vontade</span>
          </div>
          <button onClick={promptLogin} style={{ background: '#D97706', color: 'white', border: 'none', borderRadius: 8, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            Criar conta gratis
          </button>
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Placa, cliente ou numero..." className="bg-transparent text-sm outline-none flex-1 text-slate-700 placeholder-slate-400" />
        </div>
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-xl p-1">
          {['all', 'open', 'await_approval', 'in_progress', 'done'].map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filter === s ? 'bg-nexo-500 text-white shadow-nexo-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
              {s === 'all' ? 'Todas' : statusConfig[s]?.label ?? s}
            </button>
          ))}
        </div>
        <button onClick={fetchOrders} className="btn-ghost"><RefreshCw size={15} className={loading ? 'animate-spin' : ''} /></button>
        <button onClick={() => isDemoMode() ? promptLogin() : setShowModal(true)} className="btn-primary ml-auto">
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {loading && <tr><td colSpan={7} className="py-8 text-center"><Loader2 size={24} className="text-nexo-500 animate-spin mx-auto" /></td></tr>}
            {!loading && filtered.map((os) => {
              ...
              return (
                <tr key={os.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="table-cell font-mono text-xs text-slate-500">{os.vehicle_km > 0 ? `${os.vehicle_km.toLocaleString('pt-BR')} km` : '-'}</td>
                  <td className="table-cell text-center">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => isDemoMode() ? promptLogin() : undefined} className="w-7 h-7 flex items-center justify-center rounded-lg bg-nexo-50 text-nexo-600 hover:bg-nexo-100 transition-colors"><Eye size={13} /></button>
                    </div>
                  </td>
                </tr>
              )
        ...
        {!loading && filtered.length === 0 && (
          <div className="py-12 text-center">
            ...
            <button onClick={() => isDemoMode() ? promptLogin() : setShowModal(true)} className="btn-primary mt-4 mx-auto"><Plus size={14} /> Criar primeira OS</button>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-nexo-xl w-full max-w-lg animate-slide-up">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Placa *</label><input className="input font-mono uppercase" placeholder="BRA2E19" value={form.vehicle_plate} onChange={e => setForm({ ...form, vehicle_plate: e.target.value })} /></div>
                <div><label className="label">KM Atual</label><input className="input" type="number" placeholder="45200" value={form.vehicle_km} onChange={e => setForm({ ...form, vehicle_km: e.target.value })} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Modelo</label><input className="input" placeholder="Civic 2021" value={form.vehicle_model} onChange={e => setForm({ ...form, vehicle_model: e.target.value })} /></div>
                <div><label className="label">Ano</label><input className="input" type="number" placeholder="2022" value={form.vehicle_year} onChange={e => setForm({ ...form, vehicle_year: e.target.value })} /></div>
              </div>
              <div><label className="label">Cliente *</label><input className="input" placeholder="Nome ou ID do cliente" value={form.customer_id} onChange={e => setForm({ ...form, customer_id: e.target.value })} /></div>
              <div><label className="label">WhatsApp</label><input className="input" placeholder="5511999999999" value={form.customer_phone} onChange={e => setForm({ ...form, customer_phone: e.target.value })} /></div>
              <div><label className="label">Reclamacao *</label><textarea className="input resize-none" rows={3} placeholder="Descreva o problema..." value={form.complaint} onChange={e => setForm({ ...form, complaint: e.target.value })} /></div>
            </div>
            <div className="flex gap-3 px-6 py-4 border-t border-slate-100">
              <button onClick={() => setShowModal(false)} className="btn-ghost flex-1">Cancelar</button>
              <button onClick={createOS} disabled={creating || !form.vehicle_plate || !form.customer_id || !form.complaint} className="btn-primary flex-1 disabled:opacity-50">
                {creating ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />}{creating ? 'Criando...' : 'Criar OS'}
              </button>
            </div>
          </div>
        </div>
